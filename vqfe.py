...
trainable feature embedding (RY with weight-scaled inputs) + BasicEntanglerLayers.

Usage:
    python vqfe.py --train       # Train & save weights
    python vqfe.py --evaluate    # Load weights & show metrics
    python vqfe.py --predict     # Load weights & show predicted images
"""
...
ALGO_NAME   = "VQFE"
WEIGHT_FILE = os.path.join(SAVED_DIR, "vqfe_weights.pt")
WEIGHT_SHAPE = (4, N_QUBITS)
...
def circuit(inputs, weights):
    ...
    qml.BasicEntanglerLayers(weights[1:], wires=range(N_QUBITS))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description=f"{ALGO_NAME} - Variational Quantum Feature Embedding")
